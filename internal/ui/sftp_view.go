...
func (m *sftpFormModel) View() string {
	...
	if m.singlePane() {
		...
		paths = localStyle.Render(fmt.Sprintf("%s  %s", m.focusLabel(true), truncatePath(localBrowseLabel(m.localCwd, m.localShowingDrives), pathPW-ansi.StringWidth(m.focusLabel(true))-4))) + "\n" +

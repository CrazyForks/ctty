...
func TestFTPDirectDownloadSkipsConfirm(t *testing.T) {
	m := NewFTPForm(NewStyles(100), 100, 24, "site")
	m.client = &ftpclient.Client{}
	m.loading = false
	m.mode = ftpBrowse
	...
	fm := updated.(*ftpFormModel)
	if fm.mode == ftpDownloadConfirm {
		t.Fatal("d must not open confirm dialog")
	}
	if cmd == nil {
		t.Fatal("expected download cmd from direct download")
	}
	if !fm.transferring {
		t.Fatal("expected transferring after d")
	}
}

...
4. **Do not guess the target.** SSH: search → info → act. Serial/telnet:
   ...
   list them or ask.
5. **Do not print secrets.** Never read `credentials.json`, `ftp-credentials.json`,
   or dump keys. SSH passwords are in ctty's vault, not in SSH config. Telnet and
   FTP passwords are cleartext — warn the user; FTP uses a **separate** store
   (`ftp-credentials.json`), never the SSH vault.
...
FTP is cleartext by default; passwords (if saved) live in
`~/.config/ctty/ftp-credentials.json` (0600, cleartext — insecure), **not**
in the SSH `credentials.json` vault. Site inventory: `~/.config/ctty/ftp.json`.

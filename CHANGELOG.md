...
- **FTP support (first slice)** — Separate FTP transport (github.com/jlaffaye/ftp), not SFTP/SSH:
  - Site inventory in `~/.config/ctty/ftp.json` (0600); passwords in dedicated `~/.config/ctty/ftp-credentials.json` (cleartext, 0600 — documented insecure). **Not** mixed into SSH `credentials.json`.
  ...
  - Agent skill updated: agents use JSON CLI only; never open FTP TUI; warn cleartext.

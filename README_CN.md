...
- **🔄 端口转发** - 轻松设置本地、远程和动态（SOCKS）转发，带历史记录与复用
- **📝 轻松管理** - 无缝添加、编辑、移动和管理 SSH 配置；在搜索框打字后按 `a` 自动将输入内容带入添加主机表单
- **🔍 智能搜索** - 内置实时过滤和搜索，支持根据主机名、IP 地址及 `#标签` 即时匹配
...
- `↑/↓` 或 `j/k` - 浏览主机列表
- `Enter` - 连接到选中的主机
- `v` 或 `P` - **健康速览**主机实时运行状态（Uptime、CPU 负载、内存进度条、根磁盘空间）
- `Space` - **多选**切换勾选（带 `[ ]` / `[✓]` 标记，光标自动下移一行）
- `Ctrl+A` - 全选 / 全不选当前可视主机
- `Esc` - 清空已选主机（多选激活时）或退出当前模式
- `y` - 复制 SSH 连接命令到系统剪贴板（多选时按换行批量复制所有已选主机命令）
- `x` - 远程命令代码片段执行（多选时并发批量执行，弹窗聚合展示各主机执行结果与耗时）
- `p` - 探测主机网络连通性与时延（多选时仅探测已选中的主机）
...
**实时状态指示：**
- 🟢 **在线** - 低时延（<100ms）
- 🟡 **较慢 / 连接中** - 中等时延（100-300ms）或正在探测连接
- 🔴 **离线 / 超时** - 主机不可达、超时或高时延（>300ms）
- ⚫ **未知** - 连通性状态尚未确定
...
**连接：** TUI 暂停并将终端直接桥接到串口。按 `Ctrl+]` 断开并返回 TUI。
